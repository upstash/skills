...
def started(rec, events, stopkey):
    """Wall-clock ms of the recording's first frame: startedAt if it agrees with (stop - duration), else the latter."""
    s = (rec or {}).get('startedAt'); est = ev(events, stopkey) - dur(rec_path) * 1000
    if isinstance(s, str):
        try: s = datetime.fromisoformat(s.replace('Z', '+00:00')).timestamp() * 1000
        except ValueError: s = None
    if isinstance(s, (int, float)) and s < 1e12: s *= 1000
    if isinstance(s, (int, float)) and abs(s - est) < 1500: return s, 'startedAt'
    return est, 'stop-minus-duration'
