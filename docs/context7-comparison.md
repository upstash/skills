...
A second table, [Install commands](#install-commands), gives the exact command a user runs for
each row on each side.
...
The catalog above is about *format and distribution*; this table is about *what a user actually
types*. Rows are in the same order as the catalog. Commands were re-derived from each repo's own
manifests (marketplace `name`, plugin `name`, `source` path, package name) and cross-checked
against the install instructions each repo advertises in its README/docs — the source is cited
per row. `—` means the format is not present on that side; where a repo ships a manifest but
advertises no command for it, that is stated explicitly rather than invented.

| Plugin / file | Install in upstash/skills | Install in upstash/context7 |
|---|---|---|
| Agent Skill — `SKILL.md` | `npx skills add upstash/skills` (Agent Skills CLI)<br>`npx ctx7 skills install upstash/skills` (ctx7 CLI)<br><sub>README.md "Installation" → "Context7 CLI" / "Agent Skills CLI"</sub> | `npx ctx7 setup` — OAuth, generates an API key, installs the skill; `--cursor`/`--claude`/`--opencode`/`--codex` target one agent<br><sub>README.md "Installation"; `docs/clients/cursor.mdx`, `docs/clients/opencode.mdx`</sub> |
| Claude Code plugin manifest — `.claude-plugin/plugin.json` | `/plugin install upstash@upstash`<br><sub>`upstash@upstash` = `plugins[0].name`@`name` from `.claude-plugin/marketplace.json`</sub> | `/plugin install context7@context7-marketplace`<br>CLI form: `claude plugin install context7@context7-marketplace`<br><sub>`docs/clients/claude-code.mdx`; `plugins/claude/context7/README.md`</sub> |
| Claude Code marketplace manifest — `.claude-plugin/marketplace.json` | `/plugin marketplace add upstash/skills` | `/plugin marketplace add upstash/context7`<br>CLI form: `claude plugin marketplace add upstash/context7` |
| Codex plugin manifest — `.codex-plugin/plugin.json` | `codex plugin install upstash --source upstash`<br><sub>README.md "OpenAI Codex Plugin"</sub> | `codex plugin add context7@context7-marketplace`<br><sub>`docs/clients/codex.mdx`, `docs/resources/all-clients.mdx`, `plugins/codex/context7/README.md` — note the different verb/syntax, see caveat 1</sub> |
| Codex marketplace manifest — `.agents/plugins/marketplace.json` | `codex plugin marketplace add upstash/skills` | `codex plugin marketplace add upstash/context7` |
| Cursor plugin manifest — `plugin.json` | **No install command advertised.** The `.cursor-plugin/` manifests ship, but README.md documents no Cursor install (removed in commit `3fb5cc4`). | **No plugin install command advertised.** For Cursor the docs point at the CLI setup instead: `npx ctx7 setup --cursor`<br><sub>`docs/clients/cursor.mdx`</sub> |
| Cursor marketplace manifest — `.cursor-plugin/marketplace.json` | **No install command advertised** (see row above) | — |
| Cursor rule — `rules/*.mdc` | — | No separate command — bundled inside the Cursor plugin directory |
| GitHub Copilot CLI plugin manifest — `plugin.json` | — | `copilot plugin install context7@context7-marketplace`<br>interactive form: `/plugin install context7@context7-marketplace`<br><sub>`docs/clients/copilot-cli.mdx`; `plugins/copilot/context7/README.md`</sub> |
| Copilot CLI marketplace manifest — `.github/plugin/marketplace.json` | — | `copilot plugin marketplace add upstash/context7` |
| Agent Plugins portable manifest — `plugin.json` | — | `<your-agent> plugin install ./plugins/agent-plugins/context7`<br><sub>`plugins/agent-plugins/context7/README.md` — the literal command, client-agnostic by design; the only install command in either repo that hard-codes a repo-internal path</sub> |
| Agent Plugins MCP config — `mcp.json` | — | No separate command — read from the plugin directory at install time |
| Client-specific MCP server config — `.mcp.json` / `mcp.json` | No file shipped. For the separate `@upstash/mcp-server`:<br>`claude mcp add upstash -- npx -y @upstash/mcp-server@latest --email YOUR_EMAIL --api-key YOUR_API_KEY`<br><sub>README.md "MCP Server"</sub> | No separate command — bundled in each plugin. Manual equivalents:<br>`codex mcp add context7 -- npx -y @upstash/context7-mcp --api-key YOUR_API_KEY`<br><sub>`docs/resources/all-clients.mdx`</sub> |
| MCP Registry server manifest — `server.json` | — | **No user-facing install command.** The registry entry is a listing under the name `io.github.upstash/context7`; clients resolve it to the packages it declares (`npx -y @upstash/context7-mcp`) or the remote `https://mcp.context7.com/mcp`. Publishing (maintainers only) is `mcp-publisher publish` in `.github/workflows/mcp-registry.yml`. |
| Gemini CLI extension manifest — `gemini-extension.json` | — | Canonical Gemini syntax: `gemini extensions install https://github.com/upstash/context7`<br>**Not advertised by the repo** — `docs/resources/all-clients.mdx` documents manual `~/.gemini/settings.json` MCP config instead of the extension. See caveat 2. |
| Power manifest — `POWER.md` | — | **No CLI.** Kiro installs powers through the UI: Powers panel → *Add Custom Power* → *Import power from GitHub* → repo URL → *Install*. See caveat 3 — the repo URL alone would not resolve this power. |
| Subagent definition — `agents/*.md` | — | No separate command — bundled inside each plugin |
| Slash command — `commands/*.md` | — | No separate command — bundled inside each plugin; invoked as `/context7:docs` once installed |
| Plain agent rule snippets — `rules/*.md` | — | No install command — pasted by hand, or written out by `npx ctx7 setup` |
| VS Code extension | — | — (not in this repo) Published separately: install **Context7 MCP Server** from the Visual Studio Marketplace, or search `Upstash.context7-mcp` in the Extensions view<br>CLI equivalent (canonical VS Code syntax, not quoted by the docs): `code --install-extension Upstash.context7-mcp`<br><sub>`docs/clients/vscode.mdx`</sub> |

### Caveats found while deriving these
...
3. **The `POWER.md` package is probably not installable as shipped.** Kiro requires a valid
   ...
   Powers panel would therefore not find it. (Recall from the catalog that the Kiro attribution
...
4. **Repo slug form is flexible for `ctx7`.** `ctx7`'s help text says `/owner/repo`, but
   `packages/cli/src/utils/parse-input.ts` matches `^\/?([^\/]+)\/([^\/]+)# upstash/skills vs. upstash/context7 — plugin & manifest catalog
...
| VS Code extension (`package.json` with `contributes`, VSIX) | **No** | **No** — context7 only *documents* the extension in `docs/clients/vscode.mdx`; the source lives in the separate repo [upstash/context7-vscode-extension](https://github.com/upstash/context7-vscode-extension) | VS Code extension manifest (`package.json` + VSIX package) | https://code.visualstudio.com/api/references/extension-manifest | VS Code Marketplace (published as `Upstash.context7-mcp`) | https://marketplace.visualstudio.com/items?itemName=Upstash.context7-mcp |
 ...
 — the leading slash
   is optional, so upstash/skills' `npx ctx7 skills install upstash/skills` is valid as written.
